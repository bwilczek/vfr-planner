class MagDeclination < ApplicationRecord
  ...
  def self.populate
    ...
    (-179..180).each do |lng|
      (-90..90).each do |lat|
        Rails.logger.info("Fetching for lat=#{lat} lng=#{lng}")
        next if MagDeclination.find_by(lat: lat, lng: lng)
        ...
        while response.nil?
          begin
            ...
            Rails.logger.info("Error fetching data: #{e.message}, retrying after 10 seconds")
            sleep 10
          end
        end
        ...
        declination = if int_part > 0
                        int_part.to_f + fraction_part.to_f / 10**fraction_part.to_s.length
                      else
                        int_part.to_f - fraction_part.to_f / 10**fraction_part.to_s.length
                      end
        Rails.logger.info(" > declination=#{declination}")
